...
class Uploader:
    ...
    @staticmethod
    def handle_save_for_particular_tier(
        user: User, uploaded_file: InMemoryUploadedFile, link_exp_seconds: int | None
    ):
        tier = user.tier
        image = Image(creator=user, name=uploaded_file.name)
        image.save()

        def create_link_helper(
            describtion: str,
            create_or_connect: InMemoryUploadedFile | UploadImage = uploaded_file,
            resize: int = None,
            expires=None,
        ):
            if type(create_or_connect) == InMemoryUploadedFile:
                mediafile = UploadImage(mediafile=create_or_connect, resize=resize)
                mediafile.save()
        ...
        if tier.presence_of_link_to_og_file:
            og_size_file = create_link_helper(describtion="Original size image")
