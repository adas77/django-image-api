...
class TestSetUp(APITestCase):
    ...
    def helper_upload_image(self, img_path, user_authorized=None, exp_seconds=None):
        ...
        api_url += (
            f"{self.images_api_exp_seconds_query_key}{exp_seconds}"
        ...
        with open(file_path, "rb") as fp:
            data = SimpleUploadedFile(fp.name, fp.read())
            res = self.client.post(api_url, {self.key_file_upload: data})
            return res

    def helper_compare_uploaded_image_with_image_got_by_url(
        self,
        image_name_before: str,
        image_response_after: FileResponse,
        compare_height_value: int = None,
        compare_content_type=True,
        compare_width=False,
    ):
    ...
    def helper_get_expiring_link(self, user, exp_seconds):
        res = self.helper_upload_image(self.valid_image, user, exp_seconds=exp_seconds)
    ...
    def helper_mock_expiring_link_and_hit_endpoint(
        self, mock_expire_date, response_status_code_should_be
    ):
        url = self.helper_get_expiring_link(self.user_tier_enterprise, 15_000)
        url_image_id = url.split("/")[-1]
        Link.objects.filter(url=url_image_id).update(expires=mock_expire_date)
